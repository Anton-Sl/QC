...
class QcClient(object):
    ...
    url2 = "https://login.software.microfocus.com/msg/actions/doLogin.action"
   # url = "https://almalmqc1250saastrial.saas.hpe.com/qcbin"
    def __init__(self, username,password, project="350860509_DEMO"):
    ...
    def is_authenticated(self):
        r = self.session.get(self.url + '/rest/is-authenticated')
        if r.status_code == requests.codes.ok:
            return True
        elif r.status_code == requests.codes.unauthorized:
            authenticationUrl = (re.search('\"(.*)\"', r.headers.get('WWW-Authenticate'))).group(1)
    ...
    def Login(self):
        ...
        resp = self.session.post(self.url2, headers=headers,
                                 data={'username': self.username, 'password': self.password})
        print(resp.cookies)
        print(resp.status_code)
        if resp.status_code == 200:
            self.session.cookies.update({
                'LWSSO_COOKIE_KEY': resp.cookies.get('LWSSO_COOKIE_KEY'),
            })
            request = self.session.post(self.baseUrl + 'rest/site-session')
            if request.status_code == 201:
                return
        raise qc_exceptions.QCAuthenticationError("Authentication Failed [{0}]".format(resp.status_code))
    ...
    def GetEntity(self, entityType, entityId=None, query=None):
       ...
       # url = "{url}{entityType}".format(url=self.url, entityType=entityType)
        #url="https://almalmqc1250saastrial.saas.hpe.com/qcbin/rest/domains/DEFAULT_350860509/projects/350860509_DEMO/"
        url = "{url}{entityType}".format(url=self.url, entityType=entityType)
       # print(url)
        if entityId:
